...
void		draw_sprite(t_mlx *mlx, t_raycast *ray, t_player *player, t_map *map)
{
	...
	while (i >= 0)
	{
		calc_xstart(player, map, ray, ray->spr[i]);
...
void		save_pos_spr(t_raycast *ray, int x, t_map *map)
{
	...
	i = ray->nb_sprite;
	if (i != 0 && ray->spr[i - 1]->sy != x && !(ray->spr[i - 1]->sx))
	{
		ray->spr[i - 1]->sx = x;
		ray->spr[i - 1]->sy = (HAUTEUR_SCREEN / 2) - 32 * (map->dist_screen / ray->dist_spr);
	}
...
void		calc_xstart(t_player *player, t_map *map, t_raycast *ray, t_sprite *spr)
{
	...
	ray->invdet = 1.0 / (player->v0.x * player->v1.y - player->v0.y * player->v1.x);
	dist.x = (spr->mapx + 0.5) - player->pos.x;
	dist.y = (spr->mapy + 0.5) - player->pos.y;
//	tmpy = ray->invdet * (-player->v0.y * dist.x + player->v0.x * dist.y);
	tmpx = ray->invdet * (player->v1.y * dist.x - player->v1.x * dist.y);
	tmp = abs((int)(map->res_y / tmpx));
//	spr->sx = (map->res_x / 2) * (1 - tmpx / tmpy);
	spr->start = -tmp / 2 + spr->sx;
//	spr->xmax = tmp / 2 + spr->sx;
	printf("invdet: %f\ndist.x: %f\ndist.y: %f\ntmp: %f\nspr->start: %d\n_________\n", ray->invdet, dist.x, dist.y, tmp, spr->start);
}
